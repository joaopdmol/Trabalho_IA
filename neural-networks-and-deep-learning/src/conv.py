...
import network3  # Certifique-se de que network3 está adaptado para PyTorch
from network3 import load_data_shared, ConvPoolLayer, FullyConnectedLayer, SoftmaxLayer, Network
...
def dbl_conv(activation_fn=torch.sigmoid):
    for j in range(3):
    ...
    for j in range(3):
        ...
        net = Network([
            ...
            ConvPoolLayer(image_shape=(mini_batch_size, 20, 12, 12),
                          ...
                          poolsize=(2, 2),
                          activation_fn=F.relu),
            FullyConnectedLayer(n_in=40*4*4, n_out=n, activation_fn=F.relu),
...
def expanded_data_double_fc(n=100):
    """n is the number of neurons in both fully-connected layers. We'll try n=100, 300, and 1000."""
    expanded_training_data, _, _ = network3.load_data_shared("../data/mnist_expanded.pkl.gz")
    for j in range(3):
        ...
        net = Network([
            ...
            ConvPoolLayer(image_shape=(mini_batch_size, 20, 12, 12),
                          ...
                          activation_fn=F.relu),
            FullyConnectedLayer(n_in=40*4*4, n_out=n, activation_fn=F.relu),
            FullyConnectedLayer(n_in=n, n_out=n, activation_fn=F.relu),
            SoftmaxLayer(n_in=n, n_out=10)], mini_batch_size)
        net.SGD(expanded_training_data, 60, mini_batch_size, 0.03, validation_data, test_data, lmbda=0.1)
    ...
    for j in range(repetitions):
        ...
        net = Network([
            ...
            ConvPoolLayer(image_shape=(mini_batch_size, 20, 12, 12),
                          ...
                          activation_fn=F.relu),
            FullyConnectedLayer(
                n_in=40*4*4, n_out=1000, activation_fn=F.relu, p_dropout=p0),
            FullyConnectedLayer(
                n_in=1000, n_out=1000, activation_fn=F.relu, p_dropout=p1),
...
def ensemble(nets):
    ...
    test_x, test_y = test_x.to('cpu'), test_y.to('cpu')
    
    for net in nets:
        net.to('cpu')
        net.eval()
        with torch.no_grad():
            net.test_predictions = net(torch.tensor(test_x)).argmax(dim=1).numpy()
    ...
    print(f"Accuracy is {100 - len(error_locations) / 100:.2f}%")
...
def plot_errors(error_locations, erroneous_predictions=None):
    ...
    for j in range(n):
        ...
        ax.matshow(error_images[j], cmap=plt.cm.binary)
        ax.text(24, 5, str(test_y[error_locations[j]]))
        if erroneous_predictions:
            ax.text(24, 24, str(erroneous_predictions[j]))
        plt.xticks(np.array([]))
        plt.yticks(np.array([]))
    plt.tight_layout()
    return plt
...
def plot_filters(net, layer, x, y):
    ...
    filters = net.layers[layer].weight.data.cpu().numpy()
    fig = plt.figure()
...
def run_experiments():
    ...
    basic_conv()
    omit_FC()
    dbl_conv(activation_fn=torch.sigmoid)
    dbl_conv_relu()
    expanded_data(n=100)
    expanded_data(n=300)
